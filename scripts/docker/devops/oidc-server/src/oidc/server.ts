...
export class OIDCServer {
  ...
  async handleToken(
    ...
    const expectedSecrets: { [clientId: string]: string } = {
      "phoenix-oidc-client-id": "phoenix-oidc-client-id-secret-abc-123",
  ...
  private async createAccessToken(user: User, nonce?: string): Promise<string> {
    ...
    console.log(JSON.stringify(accessTokenCreation));

    const jwt = await new SignJWT(claims)
      .setProtectedHeader({ alg: "RS256", kid: "phoenix-dev-key-1" })
      .sign(this.keyPair.privateKey);

    const accessTokenComplete = {
      timestamp: new Date().toISOString(),
      event: "access_token_created",
      token_length: jwt.length,
      user_email: claims.email,
    };
    console.log(JSON.stringify(accessTokenComplete));
    return jwt;
  }
  ...
  private async createIdToken(
    ...
    const idTokenCreation = {
      timestamp: new Date().toISOString(),
      event: "id_token_creation",
      claims,
      key_id: "phoenix-dev-key-1",
      algorithm: "RS256",
      audience,
    };
    console.log(JSON.stringify(idTokenCreation));

    const jwt = await new SignJWT(claims)
      .setProtectedHeader({ alg: "RS256", kid: "phoenix-dev-key-1" })
      .sign(this.keyPair.privateKey);

    const idTokenComplete = {
      timestamp: new Date().toISOString(),
      event: "id_token_created",
      token_length: jwt.length,
      user_email: claims.email,
      audience,
    };
    console.log(JSON.stringify(idTokenComplete));
    return jwt;
  }
  ...
  // PKCE-specific methods for testing both flows
  ...
  async handlePKCEToken(
    ...
    // PKCE-specific validation: code_verifier is required
    if (!code_verifier) {
      const verifierError = {
        timestamp: new Date().toISOString(),
